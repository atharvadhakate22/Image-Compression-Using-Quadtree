...
public class CompressImage {
    ...
    public CompressImage(String inputPath, float threshold) throws IOException {
        File inputFile = new File(inputPath);
        this.img = ImageIO.read(inputFile);
        this.originalFileSize = inputFile.length();
        this.threshold = threshold;
    ...
    public void build() {
        ...
        this.qt = new QuadTree(img, this.threshold);
        long endTime = System.nanoTime();
        this.compressionTime = (endTime - startTime) / 1_000_000; // in milliseconds
    }
    ...
    public void saveCompressed(String outputPath) throws IOException {
        ...
        this.compressedFileSize = outputFile.length();
        calculateCompressionRatio();
    }

    private void calculateCompressionRatio() {
        if (originalFileSize > 0) {
            this.compressionRatio = (double) originalFileSize / compressedFileSize;
        }
    }

    public void displayCompressionInfo() {
        System.out.println("Original Image Size: " + (originalFileSize / 1024) + " KB");
        System.out.println("Compressed Image Size: " + (compressedFileSize / 1024) + " KB");
        System.out.println("Compression Ratio: " + compressionRatio);
        System.out.println("Compression Time: " + compressionTime + " ms");
        System.out.println("Max Depth of QuadTree: " + qt.maxDepth);
    }
